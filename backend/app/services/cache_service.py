...
async def check_redis() -> bool:
    ...
    通过 PING 命令验证 Redis 服务是否正常运行。

    Returns:
        bool: Redis 正常返回 True，否则返回 False
    """
...
async def get_cache(key: str) -> Optional[str]:
    ...
    try:
        ...
        print(f"Redis GET 操作失败 [{key}]: {e}")
        return None
    except Exception as e:
        print(f"缓存读取异常 [{key}]: {e}")
        return None
...
async def set_cache(key: str, value: str, ttl: int = DEFAULT_CACHE_TTL) -> bool:
    ...
    try:
        ...
        # Redis 操作失败时降级，不影响主业务
        print(f"Redis SET 操作失败 [{key}]: {e}")
        return False
    except Exception as e:
        print(f"缓存写入异常 [{key}]: {e}")
        return False
...
async def delete_cache(key: str) -> bool:
    ...
    try:
        ...
        print(f"Redis DELETE 操作失败 [{key}]: {e}")
        return False
    except Exception as e:
        print(f"缓存删除异常 [{key}]: {e}")
        return False
